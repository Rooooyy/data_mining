...
def clustering(output_path):
    embedding_df = pd.read_csv(output_path)
    embedding_df = embedding_df.drop('type', axis=1)

    tsne = TSNE()
    tsne_embedding = tsne.fit_transform(embedding_df.values)

    figure = sns.scatterplot(tsne_embedding[:,0], tsne_embedding[:,1], legend='full')
    figure.get_figure().savefig('tsne_embedding.png', dpi=400)
